...
PATTERNS = [
    # Git install URLs: ...mkdocs-modern-theme.git@v1.2.3
    (re.compile(r"mkdocs-modern-theme\.git@v[\d]+\.[\d]+\.[\d]+"),
     "mkdocs-modern-theme.git@v{version}"),
    # Inline version references: `v1.2.3`
    (re.compile(r"Replace `v[\d]+\.[\d]+\.[\d]+`"),
     "Replace `v{version}`"),
]
